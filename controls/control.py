...
def irrigation_full(client):
  ...
  timeout = 0
  T_start = time.time()
  # Check level nutrients and add  to the irrigation tank
  if level.nutrient_tank() == 2:
  ...
  time.sleep(20)
  while ((level.irrigation_tank() != 3) and (timeout < 300) and (level.water_tank(3) != 1)):       # and (level.nutrient_tank() == 2)
    time.sleep(5)
